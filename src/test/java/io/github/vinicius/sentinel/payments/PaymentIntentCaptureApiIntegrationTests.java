...
class PaymentIntentCaptureApiIntegrationTests {
	...
	@Test
	void aFullCapturePostsOneBalancedLedgerTransactionAndReachesCaptured() throws Exception {
		...
		mockMvc.perform(post("/api/v1/payment-intents/" + paymentIntentId + "/captures")
			...
			.andExpect(jsonPath("$.capturedAmountInMinorUnits").value("10000"));

		assertThat(payableDeltaSince(payableBefore)).isEqualTo(10_000L);
	}

	@Test
	void partialCapturesCanReachButNeverExceedTheAuthorizedAmount() throws Exception {
		UUID paymentIntentId = createAndAuthorize("10000");
		...
		mockMvc.perform(post("/api/v1/payment-intents/" + paymentIntentId + "/captures")
			...
			.andExpect(jsonPath("$.capturedAmountInMinorUnits").value("4000"));
	...
	@Test
	void aCaptureExceedingRemainingAuthorizationIsRejected() throws Exception {
		UUID paymentIntentId = createAndAuthorize("10000");
		...
		mockMvc.perform(post("/api/v1/payment-intents/" + paymentIntentId + "/captures")
			...
			.andExpect(jsonPath("$.code").value("CAPTURE_LIMIT_EXCEEDED"));
	}

	@Test
	void retryingWithTheSameKeyReplaysTheOriginalCaptureWithoutPostingASecondLedgerTransaction() throws Exception {
		UUID paymentIntentId = createAndAuthorize("10000");
		...
		assertThat(payableDeltaSince(payableBefore)).isEqualTo(4_000L);
	}

	@Test
	void capturingAPaymentIntentThatWasNeverAuthorizedIsRejected() throws Exception {
	...
	@Test
	void capturingAnUnknownPaymentIntentReturnsNotFound() throws Exception {
		mockMvc.perform(post("/api/v1/payment-intents/" + UUID.randomUUID() + "/captures")
				.with(httpBasic(MERCHANT_USERNAME, MERCHANT_PASSWORD))
				.header("Idempotency-Key", newIdempotencyKey())
				.contentType(MediaType.APPLICATION_JSON)
				.content("{\"amountInMinorUnits\":\"5000\",\"currency\":\"BRL\"}"))
	...
	@Test
	void capturingWithoutAnIdempotencyKeyIsRejected() throws Exception {
		UUID paymentIntentId = createAndAuthorize("10000");
